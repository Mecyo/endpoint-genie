...
const saveEndpointsToFile = (endpoints) => {
    ...
    endpoints.forEach(endpoint => {
        let json = endpoint.json;
        if(typeof endpoint.json !== 'string') {
            json = JSON.stringify(endpoint.json);
        }
        const data = `${endpoint.id}: ${endpoint.method}: ${endpoint.endpoint}: ${json}: ${endpoint.status}\n`;
...
const loadEndpointsFromFile = () => {
    ...
    if (fs.existsSync(endpointsFilePath)) {
        console.log(`Endpoints file path: ${endpointsFilePath}`);
...
app.post('/set-endpoint', (req, res) => {
    ...
    if (typeof method === 'string' && typeof endpoint === 'string' && json && !isNaN(Number(status))) {
        const id = currentId++;
